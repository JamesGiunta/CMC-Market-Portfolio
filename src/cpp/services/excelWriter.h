...
private:
    ...
    void setUpHeader(lxw_worksheet* worksheet, std::list<std::string> headers, int& row, int& col);
    ...
    void writeSoldFinancialYearShares(lxw_worksheet* worksheet, const std::vector<DataRow>& data, int& row, int& col, std::time_t previousFinancialYearEnd, std::time_t currentFinancialYearEnd, DataRow& dr);
};